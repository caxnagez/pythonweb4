...
class BookViewDialog(QDialog):
    def __init__(self, book_data, parent=None):
        ...
        pixmap = QPixmap(img_path if img_path else DEFAULT_IMAGE_PATH).scaled(200, 200, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
...
class AuthWindow(QWidget):
    ...
    def register(self):
        ...
        try:
            ...
            conn.commit()
            conn.close()
            QMessageBox.information(self, "Пользователь зарегистрирован")
        except sqlite3.IntegrityError:
            QMessageBox.warning(self, "Логин уже существует")
...
class LibraryWindow(QWidget):
    ...
    def edit_book(self):
        ...
        if current_row < 0:
            QMessageBox.warning(self, "Выберите книгу для редактирования")
            return
    ...
    def delete_book(self):
        current_row = self.booksTable.currentRow()
        if current_row < 0:
            QMessageBox.warning(self, "Выберите книгу для удаления")
            return
        book_id = int(self.booksTable.item(current_row, 0).text())
        reply = QMessageBox.question(self, "Удалить выбранную книгу?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
...
class BookDialog(QDialog):
    def __init__(self, parent=None, book_id=None, is_new=True):
        ...
        self.genre_input.addItems(["Фантастика", "Драма", "Фэнтези", "Детектив", "Роман", "Приключения", "Научная", "Триллер", "Сказка", "Другое"])
    ...
    def load_book_data(self):
        ...
        if row:
            ...
            self.year_input.setText(str(row[3]))
    ...
    def save_book(self):
        ...
        if not year_text:
            QMessageBox.warning(self, "Поле 'Год' не может быть пустым")
            return
        if not year_text.isdigit():
            QMessageBox.warning(self, "Год должен быть числом")
            return
        year = int(year_text)
        if year > datetime.now().year:
            QMessageBox.warning(self, f"Год не может быть больше текущего ({datetime.now().year})")
            return
